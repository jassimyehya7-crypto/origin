...
export function projectOnMap(lat: number, lng: number) {
  return {
    x: ((lng - VILLENEUVE_MAP.west) / (VILLENEUVE_MAP.east - VILLENEUVE_MAP.west)) * 100,
    y: ((VILLENEUVE_MAP.north - lat) / (VILLENEUVE_MAP.north - VILLENEUVE_MAP.south)) * 100,
  };
}
...
export function TownMap({
  ...
  function centerOn(px: number, py: number, s: number) {
    const vp = viewportRef.current;
    if (!vp) return;
