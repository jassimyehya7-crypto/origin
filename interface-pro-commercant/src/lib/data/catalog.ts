...
function appendNewById<T extends { id: string }>(baseline: T[], live: T[] | null) {
  const baselineIds = new Set(baseline.map((item) => item.id));
  return [...baseline, ...(live ?? []).filter((item) => !baselineIds.has(item.id))];
}
...
export function getActiveMerchants(): Merchant[] {
  return appendNewById(MERCHANTS, _supabaseMerchants);
}

/** Retourne la liste active des offres (Supabase si chargé, sinon statique) */
export function getActiveOffers(): Offer[] {
  return appendNewById(OFFERS, _supabaseOffers);
}
