...
export function createA1Offers(shops: Shop[]): Offer[] {
  ...
  const items: Array<Omit<Offer, "validUntil" | "createdAt" | "publishedAt" | "views" | "status" | "emoji"> & { age: number }> = [
    ...
    { id: "a1_kiosque_combo", shopId: "shop_kiosque", title: "Combo pause express", description: "Chips, canette et chewing-gum pour la route.", type: "FLASH", price: 5.9, originalPrice: 8.1, quantityTotal: 15, quantityLeft: 11, unit: "combo", imageUrl: "/offers/a1/kiosque-gare.webp", age: 0.75 },
  ...
  return items.map(({ age, ...offer }) => ({ ...offer, status: "PUBLIEE", emoji: "", validUntil: end(offer.shopId), createdAt: hoursAgo(age), publishedAt: hoursAgo(age), views: Math.round(24 + age * 11) }));
}
